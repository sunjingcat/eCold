...
import com.zz.cold.business.daily.adapter.DailyAdapter;
import com.zz.cold.business.storage.AddStorageActivity;
 ...
 * 日报管理
 */
public class DailyActivity extends MyBaseActivity implements OnRefreshListener, OnLoadMoreListener {
    ...
    List<DailyBean> mlist = new ArrayList<>();
    ...
    protected void initView() {
        ...
        adapter = new DailyAdapter(R.layout.item_daily, mlist);
    ...
    public void showResult(List<DailyBean> data) {
        if (pagenum == 1) {
            mlist.clear();
        }
        mlist.addAll(data);
    ...
    void getDate() {
        ...
        RxNetUtils.request(getApi(ApiService.class).getDailyList(map), new RequestObserver<JsonT<List<DailyBean>>>() {
            @Override
            protected void onSuccess(JsonT<List<DailyBean>> jsonT) {
                showResult(jsonT.getData());
            }

            @Override
            protected void onFail2(JsonT<List<DailyBean>> stringJsonT) {
